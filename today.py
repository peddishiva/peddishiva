...
def daily_readme(birthday):
    """Return the integer age; it changes only on the birthday."""
...
def rest_request(function_name, url, params=None):
    query_count("rest_repo")
    ...
    # Public repository metadata is available to fine-grained PATs even when
    # the token is restricted to selected repositories. If a selected-repo
    # restriction blocks the authenticated request, retry anonymously so
...
def graph_repos_stars(count_type, owner_affiliation):
    ...
    GitHub's current API restrictions can reject the GraphQL
    `stargazers.totalCount` field for fine-grained PATs. We therefore fetch
    repository names through GraphQL and read each repository's public
    `stargazers_count` through the REST repository endpoint.
    """
    ...
    while True:
        data = simple_request(
            ...
            {
                "login": USER_NAME,
                "affiliations": owner_affiliation,
                "cursor": cursor,
            },
        )
        ...
        if count_type == "stars":
            for edge in repos["edges"]:
                ...
                repo_data = rest_request(
                    "repository_metadata",
                    f"https://api.github.com/repos/{full_name}",
                )
                total_stars += int(repo_data.get("stargazers_count") or 0)
...
def cache_builder(edges, comment_size=0, force_cache=False):
    ...
    for edge in edges:
        ...
        target = (
            node.get("defaultBranchRef", {}).get("target")
            if node.get("defaultBranchRef")
            else None
        )
        commit_count = (
            int(target.get("history", {}).get("totalCount", 0))
            if target
            else 0
        )
...
if __name__ == "__main__":
    ...
    loc_data, loc_time = perf_counter(
        loc_query,
